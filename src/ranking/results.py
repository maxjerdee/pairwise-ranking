...
def scores(match_list, model_name="depth_and_luck", num_samples=10000, num_chains=4, force_mode=None):
    ...
    if mode == 'average': 
        ...
        for column in samples_df.columns: # Iterate over all sampled values
            if column[-6:] == "_score": # Check that this is a sampled score (not a parameter)
...
def probability(match_list, winner_label, loser_label,model_name="depth_and_luck", num_samples=10000, num_chains=4, force_mode=None):
    ...
    if mode == 'average': 
        ...
        sampled_score_differences = np.array(samples_df[f"{winner_label}_score"] - samples_df[f"{loser_label}_score"])
        ...
        match model_name:
            ...
            case "luck_only":
                alphas = np.array(samples_df["alpha"])
                betas = 100*np.ones(total_num_samples) # We use this to approximate a true step function
            case "logistic_prior":
                alphas = np.zeros(total_num_samples)
        ...
        probabilities = util.score_function(sampled_score_differences, alpha=alphas, beta=betas)
        ...
        score_difference_error = np.sqrt(MAP_score_errors[winner_index]**2 + MAP_score_errors[loser_index]**2)
        ...
        probability_upper = util.score_function(score_difference+score_difference_error, alpha=0, beta=1)
        probability_lower = util.score_function(score_difference-score_difference_error, alpha=0, beta=1)
